...
ENV CASSANDRA_TAR cassandra-db.tar.gz
...
RUN set -eux; \
	...
	rm -rf /var/lib/apt/lists/*; \
	\
#  закомментил скачивание исходников, так как используем свои
#	ddist() { \
#		local f="$1"; shift; \
...
#	rm -rf "$GNUPGHOME"; \
	...
	tar --extract --file $CASSANDRA_TAR --directory "$CASSANDRA_HOME" --strip-components 1; \
	rm $CASSANDRA_TAR; \
	\
	[ ! -e "$CASSANDRA_CONF" ]; \
